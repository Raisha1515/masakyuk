...
import 'package:masakyuk/home_page.dart';
import 'package:masakyuk/trending_page.dart';
...
class _DashboardState extends State<Dashboard> {
  ...
  ImageProvider getImageProvider(String imagePath) {
    ...
    // Network or blob URL (web)
    if (imagePath.startsWith('http') || imagePath.startsWith('https') || imagePath.startsWith('blob:')) {
      return NetworkImage(imagePath);
    }

    // On web, if not asset or network, attempt NetworkImage (some pickers emit object URLs)
    if (kIsWeb) {
      return NetworkImage(imagePath);
    }

    // Fallback: treat as local file path on mobile
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () async {
                          // tutup modal lalu buka halaman edit dan tunggu hasilnya
                          Navigator.pop(context);
                          final result = await Navigator.push(context, MaterialPageRoute(builder: (context) => TambahResep(recipe: recipe)));
                          if (result != null && result is Recipe) {
                            // pertahankan owner & privacy dari resep lama
                            result.owner = recipe.owner;
                            result.isPrivate = recipe.isPrivate;
                            final idx = recipeList.indexOf(recipe);
                            if (idx != -1) {
                              setState(() {
                                recipeList[idx] = result;
                              });
                              await _saveAllData();
                ...
                const SizedBox(height: 10),
                // Private toggle (only visible to owner)
                if (recipe.owner != null && recipe.owner == userName) ...[
                  Row(
                    children: [
                      const Icon(Icons.lock_outline, size: 18, color: Colors.grey),
                      const SizedBox(width: 8),
                      const Text('Privatkan resep untuk hanya saya'),
                      const Spacer(),
                      Switch(
                        ...
                        onChanged: (v) async {
                          ...
                          await _saveAllData();
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(v ? 'Resep diset sebagai privat' : 'Resep dibuat publik')));
                        },
                      ),
                    ],
                  ),
      ...
      children: [
        ...
        Expanded(
          child: recipeList.isEmpty
              ...
              : buildGrid(recipeList.where((r) => !r.isPrivate || r.owner == userName).toList(), isFavoritePage: false),
        ),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          IconButton(
            icon: const Icon(Icons.local_fire_department, size: 30, color: Color(0xFFFF6B24)),
            onPressed: () {
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (result != null && result is Recipe) {
                // assign owner and default privacy
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => PrivateRecipesPage(recipes: recipeList, username: userName)),
                );
              },
